...
export default function TodayHabit({ habit, token, loadTodayHabits }) {
    ...
    return (
        <TodayHabitContainer isdone={done}>
            <Information isdone={done} current={currentSequence} highest={highestSequence}>
                <h3>{name}</h3>
            ...
            <button onClick={toggleHabit}><ion-icon name="checkmark"></ion-icon></button>
        </TodayHabitContainer>
    );
}
...
const Information = styled.div`
    ...
    .highest-sequence {
        color: ${({current, highest}) =>(current === highest) && (highest > 0) ? '#8FC549' : '#666666'};
    }
`;